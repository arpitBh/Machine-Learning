...
def cluster_points(X, mu):
    ...
    for i in range(len(X)):
        ind = np.argmin(np.sum((X[i]-mu)**2,axis = 1))
        if str(ind) in clusters:
            clusters[str(ind)].append(list(X[i])) 
        else:
            clusters[str(ind)] = [list(X[i])]
...
def reevaluate_centers(mu, clusters):
    ...
    for c in sorted(clusters.keys()):
        ...
        for i in clusters[c]:
            mean[0] += i[0]
            mean[1] += i[1]
        mean[0]/=len(clusters[c])
        mean[1]/=len(clusters[c])
        newmu.append(mean)
    #print (newmu)
    # you need to fill in your solution here

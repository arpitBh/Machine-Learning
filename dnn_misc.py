...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        ##########################################################################################################################
        #print (grad.shape, X.shape, self.gradient['W'].shape)
        #print (X)
        #print (grad)
        w = self.params['W']
        backward_output = grad.dot (w.T)
        ...
        #w = self.gradient['W']
        #backward_output = grad.dot (w.T)
        #backward_output = np.dot( X, self.gradient['W'].dot(self.gradient['W'].T))
        #backward_output = np.dot (grad.dot(grad.T), X)
        #backward_output = np.dot (grad, self.gradient['W'].T)
        #backward_output[backward_output<0]=0
        """
        backward_output = np.array(
            [[-0.32766959,  0.13123228, -0.0470483 ],

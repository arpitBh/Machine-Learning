...
def objective_function(X, y, w, lamb):
    ...
    #print ((lamb/2)*np.linalg.norm(w))

    #print ((1/X.shape[0]))

    H = 1 - y*(X).dot(w).reshape((X.shape[0],))
    H[H<0] = 0
    #print (y.shape, H.shape)
    obj_value = (lamb/2)*(np.linalg.norm(w)**2) + (1/X.shape[0])*np.sum(H)
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    """
    #print (objective_function(Xtrain,ytrain,w,lamb))
    #print (w.shape)
    np.random.seed(0)
    ...
    for iter in range(1, max_iterations + 1):
        ...
        X_new = Xtrain.take(A_t, axis = 0)
        y_new = ytrain.take(A_t, axis = 0)

        H = y_new*(X_new).dot(w).reshape((X_new.shape[0],))
        A_t_plus = [i for i,v in enumerate(H < 1) if v]
        X_new = X_new.take(A_t_plus, axis = 0)
        y_new = y_new.take(A_t_plus, axis = 0)
        #print (indices)
        K = len(A_t_plus)
        t_y = y_new.reshape((y_new.shape[0],1))
        #print (t_y)
        #print ()
        lr = (1.0/(lamb*iter))
        sum_columns = np.sum(np.multiply(X_new,t_y),axis=0)
        #print (((1.0 - lamb*lr)*w).shape)
        #print (sum_columns, sum_columns.shape)
        w_int = (1.0 - lamb*lr)*w + (lr/k)*sum_columns.reshape((sum_columns.shape[0],1))
        #print (w_int.shape)

        min_sc = min(1,(1/np.sqrt(lamb))/np.linalg.norm(w_int))
        w = min_sc*w_int
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    H = Xtest.dot(w).reshape(-1)
    
    #print (H.shape, ytest.shape)
    H[H<0]=-1
    H[H>0]=1
    #print (H)
    test_acc = np.sum(H==ytest)/ytest.shape[0]

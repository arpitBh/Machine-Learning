...
def main(main_params):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            # TODO (2)
            a_new = model['C2'].forward(m1)
            h_new = model['nonlinear2'].forward(a_new)
            m2 = model['M2'].forward(h_new)
            ...
            # TODO (3)
            grad_h_new = model['M2'].backward(h_new, grad_m2)
            grad_a_new = model['nonlinear2'].backward(a_new, grad_h_new)
            grad_m1 = model['C2'].backward(m1, grad_a_new)
